...
output_csv = "/home/phastos/Projects/data/ifilella/ProtSeq2StrucAlpha/structures/all_proteins.csv"

os.makedirs(output_dir, exist_ok=True)

with open(output_csv, "w", newline="") as file:
    writer = csv.writer(file)
    writer.writerow(["ID", "aa_seq", "struc_seq"])

    with alive_bar(len(pdb_path), bar="fish") as bar:
        for pdb in pdb_path:
            raw_data = get_struc_seq(
                "/home/phastos/Projects/data/ifilella/ProtSeq2StrucAlpha/bin/foldseek",
                pdb,
                chains=["A"]
            )
            if "A" in raw_data:
                ...
                writer.writerow([pdb_suffix, aa_seq, struc_seq])
